...
export function WorkspaceReportsPage(): ReactElement {
  return (
    <div
      className="w-full min-w-0 bg-[var(--track-surface)] px-2 py-2 text-white"
      data-testid="reports-page"
    >
      <section className="rounded-[14px] border border-[#32323a] bg-[#232328]">
        <div className="border-b border-[#32323a] px-4 pt-3">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                className="h-9 rounded-[10px] bg-[#b78cc8] px-4 text-[13px] font-medium text-[#17151a]"
                type="button"
              >
                Save and share
              </button>
          ...
          <div className="flex flex-wrap items-center gap-2 py-3" data-testid="reports-filter-bar">
            <div className="flex overflow-hidden rounded-[10px] border border-[#34343a] bg-[#202025] text-[13px]">
              <button className="h-8 px-4 text-[#b2b3be]" type="button">
                Prev
              </button>
              <button
                className="h-8 border-x border-[#34343a] px-4 font-medium text-white"
                type="button"
              >
                This week . W12
              </button>
      ...
      <section
        className="mt-2 grid grid-cols-4 overflow-hidden rounded-[14px] border border-[#32323a] bg-[#232328]"
        data-testid="reports-summary-metrics"
      >
        {SUMMARY_METRICS.map((metric, index) => (
          <div
            className={`px-4 py-4 ${index === SUMMARY_METRICS.length - 1 ? "" : "border-r border-[#32323a]"}`}
            key={metric.title}
          >
            <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-[#70758a]">
              {metric.title}
            </p>
            <p className="mt-3 text-[18px] font-semibold tabular-nums text-white">{metric.value}</p>
          </div>
        ))}
      </section>

      <div className="mt-2 grid grid-cols-[minmax(0,1fr)_238px] gap-2">
        <section
          className="rounded-[14px] border border-[#32323a] bg-[#232328] p-4"
          data-testid="reports-duration-chart"
        >
          <h2 className="text-[15px] font-semibold text-white">Duration by day</h2>
          <div className="mt-4 rounded-[12px] border border-[#32323a] bg-[#1f1f23] px-4 pb-9 pt-3">
            <div className="grid h-[248px] grid-cols-[44px_minmax(0,1fr)] gap-4">
              <div className="flex flex-col justify-between pb-6 text-[11px] font-medium text-[#70758a]">
              ...
              <div className="relative">
                <div className="pointer-events-none absolute inset-0 flex flex-col justify-between pb-6">
                  {Y_AXIS_LABELS.map((label) => (
                    <div className="border-t border-dashed border-[#2f3138]" key={label} />
                  ))}
                </div>
                <div className="relative z-10 flex h-full items-end gap-4 pb-6">
                  {DAY_ROWS.map((row) => (
                    <div
                      ...
                      key={row.label}
                    >
                      <span className="text-[10px] font-medium tabular-nums text-[#70758a]">
                        {row.value}
                      </span>
                      <div className="flex h-full w-full items-end">
                        <div
                          className="w-full rounded-t-md bg-[#a15cb3]"
                          style={{
                            height:
                              row.minutes === 0
                                ? "2px"
                      ...
                      <span className="text-[10px] font-medium text-[#70758a]">{row.label}</span>
                    </div>
                  ))}
                </div>
                <div className="absolute bottom-0 left-1/2 flex -translate-x-1/2 items-center gap-2 text-[11px] text-[#70758a]">
                  <span className="h-1.5 w-4 rounded-full bg-[#a15cb3]" />
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <h2 className="max-w-[92px] text-[15px] font-semibold leading-5 text-white">
          ...
          <div className="mt-8 flex justify-center">
            ...
            >
              <div className="flex size-[106px] flex-col items-center justify-center rounded-full bg-[#232328] text-center">
                <p className="text-[18px] font-semibold tabular-nums text-white">53:49:15</p>
                <p className="mt-2 text-[10px] font-medium uppercase tracking-[0.22em] text-[#70758a]">
                  Project
                </p>
              </div>
            </div>
      ...
      <section
        className="mt-2 rounded-[14px] border border-[#32323a] bg-[#232328] p-4"
        data-testid="reports-breakdown-panel"
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-[15px] font-semibold text-white">Project and member breakdown</h2>
            <p className="mt-2 text-[13px] text-[#70758a]">
        ...
        <div
          className="mt-4 overflow-hidden rounded-[12px] border border-[#32323a] bg-[#1f1f23]"
          data-testid="reports-breakdown-table"
        >
          <div className="grid grid-cols-[28px_minmax(0,1fr)_98px_98px_24px] items-center gap-3 border-b border-[#32323a] px-4 py-3 text-[11px] font-medium uppercase tracking-[0.18em] text-[#70758a]">
            <span />
          ...
          <ul>
            {BREAKDOWN_ROWS.map((row) => (
              <li
                className="grid grid-cols-[28px_minmax(0,1fr)_98px_98px_24px] items-center gap-3 border-b border-[#2b2d33] px-4 py-4 last:border-b-0"
                key={row.name}
              >
                <button className="text-left text-[13px] text-[#7e8191]" type="button">
                  &gt;
                </button>
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[13px] text-[#70758a]">({row.memberCount})</span>
                  </div>
                </div>
  ...
  return (
    <button
      ...
      className={`border-b-2 pb-3 text-[14px] font-medium ${
        active ? "border-[#b77dcb] text-[#d8b0e4]" : "border-transparent text-[#9497a7]"
      }`}
      type="button"
    >
      {children}
...
function ToolbarButton({ children }: { children: string }) {
  return (
    <button
      className="h-9 rounded-[10px] border border-[#34343a] bg-[#202025] px-3 text-[13px] font-medium text-[#d3d4dc]"
      type="button"
    >
      {children}
    </button>
