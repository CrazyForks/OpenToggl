...
test.describe("Story: user switches timezone", () => {
  ...
  test("Given a running timer, when the user views the timer bar, then the start time is visible and formatted in the user's timezone", async ({
    page,
  }) => {
    const email = `tz-running-${test.info().workerIndex}-${Date.now()}@example.com`;
    await registerE2eUser(page, test.info(), {
      email,
      fullName: "Tz Running User",
      password: "secret-pass",
    });

    // Set timezone deterministically.
    await page.goto(new URL("/account", page.url()).toString());
    await expect(page.getByTestId("account-page")).toBeVisible();
    ...
    // Start a timer directly via API — entry with no stop is "running".
    const startUtc = "2030-06-10T03:15:00.000Z"; // 12:15 Asia/Tokyo
    await page.evaluate(
      async ({ start, workspaceId: wid }) => {
        await fetch(`/api/v9/workspaces/${wid}/time_entries`, {
          body: JSON.stringify({
            created_with: "playwright-e2e",
            description: "Running tz entry",
            duration: -1,
            start,
            workspace_id: wid,
          }),
      ...
      { start: startUtc, workspaceId },
    );

    await page.goto(new URL("/timer", page.url()).toString());
    await expect(page.getByTestId("app-shell")).toBeVisible();

    // The start time of a running entry should appear near the elapsed counter.
    // We don't pin the exact format — just that "12:15" (Tokyo) shows, and the
    // UTC hour "03:15" does not.
    await expect(page.getByTestId("timer-running-started-at")).toContainText("12:15");
    await expect(page.getByTestId("timer-running-started-at")).not.toContainText("03:15");
  });
});
