...
- 如果新目标只是“短期继续沿用现有交付方式”，本方案仍可作为过渡
- 但如果新目标已经提升为：正式版单二进制启动时自动判断并执行 migration，且正式支持从 `v0.0.17` 的现状迁移到新体系，那么本方案不能完整满足该目标
- 原因不是它完全不能自动 apply schema，而是它仍然缺少正式的版本化桥接历史、明确的升级状态记录，以及把 `v0.0.17` 现状安全收口到新世界的受控机制
...
- 优先保持现有流程和文档不大改
- 接受复杂迁移继续依赖人工控制
- 接受它不能作为“从 `v0.0.17` 进入新 migration 体系”的正式长期方案
...
如果 OpenToggl 只是想降低当前改造风险、短期不动 schema 工具链，这个方案可作为过渡；如果目标已经是“正式版启动自动迁移 + 支持从 `v0.0.17` 收口到新 migration 体系”，这不是终局方案，也不应被视为完整满足目标的主方案。
