...
测试资产的长期命名只允许表达产品能力、合同边界、运行时边界或故事目标；阶段语义只能留在计划和归档中，不能成为测试名、fixture 名或生成合同测试名的正式命名边界。
...
| Transport Contract      | 公开 API 与错误语义                        | `apps/backend/internal/http/**/*_test.go`、`apps/backend/internal/*/transport/http/**/*_test.go`、`apps/website/src/shared/api/__tests__/*` | 路径、字段、状态码、错误映射   |
...
- 响应字段 shape
- 错误码与错误 body
- 外部公开 API 与 Web 接口的差异

来源规则：

- 由 `toggl-*` OpenAPI 驱动的外部公开 API，其 contract test 以对应用户故事或明确规则为入口，并以 `toggl-*` OpenAPI 为合同校验来源
